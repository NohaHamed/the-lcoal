...
clearCart.addEventListener('click', function() {
                ...
                localStorage.setItem("cartItemsNum", JSON.stringify(cartNum));
            })
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    if(storedCart.length > 0) {
        cartContent.innerHTML += storedCart.map(item => {
...
total.innerText = cartArr.map(item => item.price).reduce((a,b)=>a+b).toFixed(2);
...
// --Shopping Cart display and functions------------------------
    ...
    addBtns.forEach(function(btn)  {
        btn.addEventListener('click', function(e) {
            ...
            cartContent.innerHTML += 
                `<tr id="${clickedItem.id}">
                    <td>${clickedItem.title}</td>
                    <td>${(clickedItem.price).toFixed(2)}</td>
                    <td><button class="remove"><i class="fas fa-trash-alt"></i></button></td>
                </tr>`;
                let newCartObj = {
...
// -----Showing the cart total and number of items-----------------------------
            ...
            let totalPrice = cartArr.map(item => item.price).reduce((a,b) => a+b).toFixed(2);
...
// ---remove items from cart -------------------------------
            ...
            removeBtns.forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    totalPrice = cartArr.map(item => item.price).reduce((a,b) => a+b).toFixed(2);
                    total.innerText = totalPrice;
                    let cartNum = cartArr.length   
                    itemsNum.innerText = cartNum;
...
// ___Update payment page - Added Items________________________________
            localStorage.setItem("cart", JSON.stringify(cartArr));
            localStorage.setItem("cartItemsNum", JSON.stringify(cartNum));
        })
    })
